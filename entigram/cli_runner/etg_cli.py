...
def main():
    ...
    deliver_parser.add_argument(
        "--expectation",
        help="Filter to a specific expectation name (informational only)",
    )
    deliver_parser.add_argument(
        "--proof",
        action="append",
...
"""
        ...
        if args.broker_command == "decide":
            ...
            if args.json_output:
                ...
                if result["valid"]:
                    ...
                    print(f"\n📦 Delivery snapshot recorded: {snap}")
        ...
        if args.improve_command == "propose":
            ...
            if not proposals:
                ...
                for p in proposals:
                    print(f"  [{p['lifecycle_status']}] #{p['id']} {p['title']}")
